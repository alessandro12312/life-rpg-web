...
export class LogActivityDto {
    ...
    @Max(480, { message: 'Non puoi registrare più di 8 ore (480 min) in una sola sessione per evitare abusi' })
    duration_minutes: number;

    @IsString()
    @IsNotEmpty()
    @IsIn(['intelligence', 'strength', 'endurance', 'discipline', 'focus', 'knowledge', 'health'])
