...
export class BattleService {
  ...
  async createBoss(
    ...
    const scaled = scaleBossStats(creatorLevel, dto.tier, dto.difficulty_factor, 0);

    const xpReward = Math.floor(
      100 * dto.tier * dto.difficulty_factor * phaseCount,
    );
    ...
    if (error)
      throw new InternalServerErrorException('Errore nella creazione del boss');
    return data;
  }
  ...
  async listBosses(userId: string) {
    ...
    if (error)
      throw new InternalServerErrorException('Errore nel recupero dei boss');
    return data ?? [];
  }

  /** Delete a boss definition */
  async deleteBoss(userId: string, bossId: string) {
    const client = this.supabase.getClient();

    // Verify user is creator
    const { data: boss, error: fetchError } = await client
      .from('bosses')
      .select('creator_id')
      .eq('id', bossId)
      .single();

    if (fetchError || !boss) throw new NotFoundException('Boss non trovato');
    if (boss.creator_id !== userId) {
      throw new ForbiddenException('Non puoi eliminare questo boss');
    }
    ...
    if (deleteError) {
      throw new InternalServerErrorException('Errore nell\'eliminazione del boss');
    }
    return { success: true };
  }
  ...
  async startBattle(userId: string, dto: { boss_id: string; mode?: string }) {
    ...
    if (activeBattles && activeBattles.length > 0) {
      throw new BadRequestException(
        'Sei già in una battaglia attiva. Concludila o abbandonala prima.',
      );
    }
    ...
    const { data: userRow } = await client
      ...
      .eq('id', userId)
      .single();
    const playerLevel = userRow?.level || 1;

    // Scale boss stats for phase 1
    const phase1Stats = scaleBossStats(playerLevel, boss.tier, boss.difficulty_factor, 0);

    const initialStatus = mode === 'SOLO' ? 'ACTIVE' : 'WAITING';
    const startedAt = mode === 'SOLO' ? new Date().toISOString() : null;
    ...
    if (battleError || !battle)
      throw new InternalServerErrorException(
        'Errore nella creazione della battaglia',
      );
    ...
    // Set active participant (only for SOLO, in PARTY it will be set when combat starts)
    await client
      .from('battles')
      .update({ active_participant_id: mode === 'SOLO' ? userId : null })
      .eq('id', battle.id);

    return this.getBattleState(battle.id, userId);
  }

  /** Get current active battle for user */
  async getActiveBattle(userId: string) {
    const client = this.supabase.getClient();

    const { data: activeParticipant } = await client
      .from('battle_participants')
      .select('battle_id, battles!inner(id, boss_id, status, mode)')
      .eq('user_id', userId)
      .in('battles.status', ['WAITING', 'ACTIVE'])
      .maybeSingle();
    ...
    const battle: any = activeParticipant.battles;
    return {
      battleId: battle.id,
  ...
  async joinBattle(userId: string, battleId: string) {
    ...
    if (battle.status !== 'WAITING') {
      throw new BadRequestException(
        'Puoi unirti a una battaglia solo durante la fase di lobby (WAITING)',
      );
    }
    if (battle.mode === 'SOLO') {
    ...
    const { count } = await client
      ...
      .eq('battle_id', battleId);
    if ((count || 0) >= 4)
      throw new BadRequestException('La battaglia è piena (max 4 giocatori)');
  ...
  async startCombat(userId: string, battleId: string) {
    const client = this.supabase.getClient();
    ...
    const { data: battle, error: battleError } = await client
      ...
      .single();
    if (battleError || !battle)
      throw new NotFoundException('Battaglia non trovata');

    if (battle.status !== 'WAITING') {
      throw new BadRequestException(
        'Il combattimento è già iniziato o terminato',
      );
    }

    // Verify user is host (turn_order = 1)
    const { data: hostParticipant, error: hostError } = await client
      .from('battle_participants')
      .select('*')
      .eq('battle_id', battleId)
      .eq('user_id', userId)
    ...
    if (hostError || !hostParticipant || hostParticipant.turn_order !== 1) {
      throw new ForbiddenException(
        "Solo l'host della battaglia può iniziare il combattimento",
      );
    }

    // Get average level of all participants
    const avgLevel = await this.getBattleLevel(battleId);

    // Fetch boss info to scale stats
    const { data: battleObj } = await client
      .from('battles')
      .select('*, bosses(*)')
      .eq('id', battleId)
      .single();

    const bossObj = battleObj?.bosses;
    const scaledStats = scaleBossStats(
      avgLevel,
      bossObj?.tier || 1,
      bossObj?.difficulty_factor || 1.0,
      0,
    );

    // Update battle status to ACTIVE, started_at = NOW(), and active_participant_id = host
    await client
      .from('battles')
      .update({
        status: 'ACTIVE',
        started_at: new Date().toISOString(),
        active_participant_id: userId,
        boss_current_hp: scaledStats.hp,
        boss_max_hp: scaledStats.hp,
        boss_atk: scaledStats.atk,
        boss_def: scaledStats.def,
      })
      .eq('id', battleId);

    // Add a log entry that combat has started
    await client.from('battle_logs').insert({
      battle_id: battleId,
      turn_number: 1,
      actor_type: 'BOSS',
      action_type: 'BOSS_ATTACK',
      narrative: '⚔️ Il combattimento ha inizio! Gli eroi caricano il Boss!',
    });

    return this.getBattleState(battleId, userId);
  }

  /** List open party battles (mode = PARTY, status = WAITING) */
  async listOpenPartyBattles(userId: string) {
    const client = this.supabase.getClient();

    // Fetch battles in WAITING status with mode PARTY or RAID
    const { data: battles, error } = await client
      .from('battles')
      .select('*, bosses(name, tier, difficulty_factor, phase_count)')
      .eq('status', 'WAITING')
      .in('mode', ['PARTY', 'RAID'])
      .order('created_at', { ascending: false });
    ...
    if (error)
      throw new InternalServerErrorException(
        'Errore nel recupero delle lobby di battaglia',
      );

    // For each battle, we also want to fetch participants to show count and if current user is inside
    const result = [];
    for (const battle of battles || []) {
      const { data: participants } = await client
        .from('battle_participants')
        .select('user_id, turn_order, users(username, avatar_id)')
        .eq('battle_id', battle.id);

      const isParticipant = (participants || []).some(
        (p: any) => p.user_id === userId,
      );
      const host: any = (participants || []).find(
        (p: any) => p.turn_order === 1,
      );
      const hostUser = host?.users
        ? Array.isArray(host.users)
          ? host.users[0]
          : host.users
        : null;
      ...
      result.push({
        ...
        boss: {
          ...
          difficulty_factor: battle.bosses?.difficulty_factor || 1.0,
          phase_count: battle.bosses?.phase_count || 1,
        },
        ...
        host: hostUser
          ? {
              username: hostUser.username || 'Unknown',
              avatar_id: hostUser.avatar_id,
            }
          : null,
      });
    }
  ...
  async submitAction(
    ...
    if (battle.status !== 'ACTIVE')
      throw new BadRequestException('La battaglia non è attiva');
    ...
    if (participant.current_hp <= 0)
      throw new BadRequestException('Il tuo personaggio è KO');
    ...
    const effectiveAtk = applyStatusEffects(
      participant.atk,
      'atk',
      statusEffects,
    );
    const effectiveDef = applyStatusEffects(
      participant.def,
      'def',
      statusEffects,
    );
    ...
    switch (dto.action) {
      case 'ATTACK': {
        const isMiss = rollMiss(
          applyStatusEffects(participant.spd, 'spd', statusEffects),
          10,
        );
      ...
      case 'SKILL': {
        if (!dto.skill_id)
          throw new BadRequestException('skill_id è richiesto per le abilità');
        ...
        if (
          !skill.classes.includes('*') &&
          !skill.classes.includes(className)
        ) {
          throw new ForbiddenException(
            'La tua classe non può usare questa abilità',
          );
        }
        ...
        turnLogs.push({
          ...
          target_type:
            skill.effect.type === 'HEAL' || skill.effect.type === 'BUFF'
              ? 'PLAYER'
              : 'BOSS',
          target_id:
            skill.effect.type === 'HEAL' || skill.effect.type === 'BUFF'
      ...
      case 'DEFEND': {
        ...
        turnLogs.push({
          ...
          action_type: 'DEFEND',
          narrative:
            'Ti metti in posizione difensiva! Danno ridotto del 50% questo turno.',
        });
        break;
      }
      ...
      case 'ITEM': {
        if (!dto.item_id)
          throw new BadRequestException(
            'item_id è richiesto per usare un oggetto',
          );
    ...
    if (battle.mode !== 'SOLO') {
      const aliveParticipants = await this.getAliveParticipants(battleId);
      const nextParticipant = await this.getNextActiveParticipant(
        battleId,
        userId,
      );
      if (nextParticipant) {
        const currentIdx = aliveParticipants.findIndex((p: any) => p.user_id === userId);
        const nextIdx = aliveParticipants.findIndex((p: any) => p.user_id === nextParticipant.user_id);
        isRoundComplete = nextIdx <= currentIdx || aliveParticipants.length <= 1;
      }
    }

    // ─── Boss Turn (if still alive, not victory, and round is complete) ──
    if (isRoundComplete && bossHp > 0 && battleStatus === 'ACTIVE') {
      ...
      if (targetParticipant) {
        ...
        if (bossAction.targetId === userId) {
          ...
          const newTargetHp = Math.max(
            0,
            targetParticipant.current_hp - bossDamage,
          );
          await client
            .from('battle_participants')
    ...
    // ─── Determine next active participant after boss turn ──
    let nextActiveParticipantId = userId;
    if (battle.mode !== 'SOLO' && battleStatus === 'ACTIVE') {
      const nextParticipant = await this.getNextActiveParticipant(
        battleId,
        userId,
      );
      if (nextParticipant) {
        nextActiveParticipantId = nextParticipant.user_id;
      }
    }
    ...
    if (isRoundComplete) {
      battleUpdate.current_turn = battle.current_turn + 1;
    } else {
      battleUpdate.current_turn = battle.current_turn;
    }

    if (battle.mode !== 'SOLO') {
      battleUpdate.active_participant_id = nextActiveParticipantId;
    }
  ...
  async abandonBattle(userId: string, battleId: string) {
    ...
    const { data: battle } = await client
      .from('battles')
      .select('status, mode, active_participant_id')
      .eq('id', battleId)
      .single();
    if (!battle) throw new NotFoundException('Battaglia non trovata');
    ...
    if (battle.mode === 'SOLO') {
      ...
      // Party: check if user is the active participant
      const isActive = battle.active_participant_id === userId;

      // If active, we need to pass the turn to the next player BEFORE deleting them
      if (isActive) {
        const nextParticipant = await this.getNextActiveParticipant(
          battleId,
          userId,
        );
        if (nextParticipant && nextParticipant.user_id !== userId) {
          await client
            .from('battles')
            .update({ active_participant_id: nextParticipant.user_id })
            .eq('id', battleId);
        }
      }
  ...
  async convertToSolo(userId: string, battleId: string) {
    const client = this.supabase.getClient();
    ...
    if (battle.status !== 'WAITING') {
      throw new BadRequestException('Puoi convertire in Solo solo durante la fase di lobby (WAITING)');
    }

    // Verify user is host (turn_order = 1)
    const { data: hostParticipant } = await client
      .from('battle_participants')
      .select('*')
      .eq('battle_id', battleId)
      .eq('user_id', userId)
    ...
    if (!hostParticipant || hostParticipant.turn_order !== 1) {
      throw new ForbiddenException('Solo l\'host della lobby può convertire la battaglia');
    }
    ...
    // Update battle to SOLO and ACTIVE
    await client
      .from('battles')
      .update({
        mode: 'SOLO',
    ...
    await client.from('battle_logs').insert({
      ...
      actor_type: 'BOSS',
      action_type: 'BOSS_ATTACK',
      narrative: '📝 La battaglia è stata convertita in modalità Solo. Combattimento avviato!',
    });
  ...
  async convertToParty(userId: string, battleId: string) {
    const client = this.supabase.getClient();
    ...
    if (battle.mode !== 'SOLO' || battle.status !== 'ACTIVE') {
      throw new BadRequestException('Puoi convertire in Party solo una battaglia Solo attiva');
    }
    ...
    // Update battle to PARTY and WAITING
    await client
      .from('battles')
      .update({
        mode: 'PARTY',
    ...
    await client.from('battle_logs').insert({
      ...
      actor_type: 'BOSS',
      action_type: 'BOSS_ATTACK',
      narrative: '📝 La battaglia è stata convertita in modalità Party ed è tornata in fase di lobby.',
    });
  ...
  async getBattleLog(battleId: string, userId: string) {
    ...
    if (error)
      throw new InternalServerErrorException(
        'Errore nel recupero del battle log',
      );
    return data ?? [];
  }
  ...
  private async derivePlayerCombatStats(userId: string) {
    ...
    if (!user || !stats)
      throw new NotFoundException('Player stats non trovati');
  ...
  private async getBattleLevel(battleId: string): Promise<number> {
    const client = this.supabase.getClient();
    const { data: participants } = await client
      .from('battle_participants')
      .select('user_id, users(level)')
      .eq('battle_id', battleId);

    if (!participants || participants.length === 0) return 1;

    const levels = participants.map((p: any) => {
  ...
  private async getNextActiveParticipant(
    battleId: string,
  ...
  private processSkillEffect(
    ...
    switch (skill.effect.type) {
      ...
      case 'HEAL': {
        healingDone = calculateHealing(
          playerMaxHp,
          playerHp,
          skill.effect.multiplier || 0.3,
        );
      ...
      case 'BUFF': {
        ...
        const statLabel =
          skill.effect.statAffected === 'atk'
            ? 'Attacco'
            : skill.effect.statAffected === 'def'
              ? 'Difesa'
              : 'Velocità';
  ...
  async distributeBattleRewards(battleId: string, status: string) {
    const client = this.supabase.getClient();
  ...
  private generateLootTable(
    tier: number,
  ): Array<{ itemId: string; dropRate: number }> {
