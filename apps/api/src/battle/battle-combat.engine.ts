...
export function deriveCombatStats(level: number, stats: CharacterStats): CombatStats {
    const s = stats;
    return {
...
// ─── Boss Stat Scaling ──────────────────────────────────────────────────────
// Calculates boss stats for a specific phase.
export function scaleBossStats(
    ...
    return {
        ...
        def: Math.floor((10 + tier * 8) * difficultyFactor * (1 + phaseIndex * 0.1)),
    };
}
...
export function calculateDamage(
    ...
    return Math.floor(baseDamage * skillMultiplier * variance * critMultiplier * defendMultiplier);
}

// ─── Healing Calculation ────────────────────────────────────────────────────
export function calculateHealing(maxHp: number, currentHp: number, healMultiplier: number): number {
...
export function decideBossAction(
    bossName: string,
    bossAtk: number,
    participants: Array<{ userId: string; currentHp: number; maxHp: number; isAlive: boolean }>,
    currentPhase: number,
    turnNumber: number,
): BossAction {
    ...
    if (Math.random() < 0.7) {
        target = alive.reduce((min, p) => p.currentHp < min.currentHp ? p : min, alive[0]);
    } else {
    ...
    if (useSpecial) {
        const specialNames = [
            'Colpo Devastante', 'Artiglio Infernale', 'Soffio di Fuoco',
            'Schianto Sismico', 'Lama Oscura', 'Morso Velenoso',
        ];
