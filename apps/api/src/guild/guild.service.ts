...
const WEEKLY_QUEST_TEMPLATES = [
  {
    ...
    description: 'Accumulate minuti di studio collettivamente.',
    category: 'STUDY',
    target_minutes: 500,
    xp_reward: 500,
  },
  {
    title: 'Guerrieri del Fitness',
    description: 'Accumulate minuti di allenamento collettivamente.',
    category: 'WORKOUT',
    target_minutes: 300,
    xp_reward: 500,
  },
  {
    title: 'Sfida Mista',
    description: 'Accumulate minuti di qualsiasi attività collettivamente.',
    category: 'MIXED',
    target_minutes: 600,
    xp_reward: 750,
  },
...
export class GuildService {
  ...
  async listGuilds() {
    ...
    if (error)
      throw new InternalServerErrorException(
        'Errore nel caricamento delle gilde',
      );
  ...
  async createGuild(
    userId: string,
    dto: { name: string; description?: string; motto?: string },
  ) {
    ...
    if (existing)
      throw new BadRequestException(
        'Sei già membro di una gilda. Abbandonala prima di crearne una nuova.',
      );
    ...
    if (error) {
      if (error.code === '23505')
        throw new BadRequestException('Una gilda con questo nome esiste già.');
      throw new InternalServerErrorException(
        'Errore nella creazione della gilda',
      );
    }
  ...
  async getGuildDetail(guildId: string) {
    ...
    const { data: members } = await client
      .from('guild_members')
      .select(
        'user_id, role, joined_at, users(username, avatar_url, level, class_name, race)',
      )
      .eq('guild_id', guildId)
  ...
  async joinGuild(userId: string, guildId: string) {
    ...
    if ((count || 0) >= guild.max_members)
      throw new BadRequestException('La gilda è piena.');
    ...
    if (error)
      throw new InternalServerErrorException('Errore nel join alla gilda');
  ...
  async leaveGuild(userId: string, guildId: string) {
    ...
    if (!membership)
      throw new NotFoundException('Non sei membro di questa gilda');
      ...
      if (officers && officers.length > 0) {
        ...
        if ((count || 0) === 0) {
          ...
          if (oldest && oldest.length > 0) {
            await client
              .from('guilds')
              .update({ leader_id: oldest[0].user_id })
              .eq('id', guildId);
            await client
              .from('guild_members')
              .update({ role: 'LEADER' })
              .eq('guild_id', guildId)
              .eq('user_id', oldest[0].user_id);
          }
        }
      }
    }
  ...
  async kickMember(requesterId: string, guildId: string, targetUserId: string) {
    ...
    if (
      !requester ||
      (requester.role !== 'LEADER' && requester.role !== 'OFFICER')
    ) {
      throw new ForbiddenException(
        'Solo il Leader o un Officer possono espellere membri',
      );
    }
    ...
    if (target.role === 'LEADER')
      throw new ForbiddenException('Non puoi espellere il Leader');
  ...
  async promoteMember(
    ...
    if (target.role !== 'MEMBER')
      throw new BadRequestException('Questo utente è già Officer o Leader');
   ...
   */
  async updateGuildQuestProgress(
    userId: string,
    category: string,
